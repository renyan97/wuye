...
<%
	  ...
	  try
	  {
          ...
          ResultSet rs = sql.executeQuery("select * from guanli where type='tc';");
		   int s=0;
		   %>
...
<p Class='r2'>
			 <a Class='r8' href='guanli_xiugai.jsp?g_num=<%=rs.getString("num")%>&g_type=<%=rs.getString("type")%>&g_no=<%=rs.getString("no")%>&g_Etime=<%=rs.getString("Etime")%>&g_degree=<%=rs.getString("degree")%>&g_price=<%=rs.getString("price")%>&g_name=<%=rs.getString("name")%>&g_state=<%=rs.getString("state")%>'>修改</a> 
			  <a Class='r9' href='guanli_shanchu.jsp?id=<%=rs.getString("no")%>' onclick="deleteJobDetail()">删除</a> </p>
			   </div>
